...
from net_utils import ChannelWeights, CrossBlock
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        """
        MLP Block: input: x with size(B N C); output size (B N C')
        """
...
class HybridTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=20, num_classes=2, embed_dims=[64, 128, 256, 512],
    ...
    def forward_features(self, x_eeg, x_fnirs):
        ...
        # B H*W/64 C
        x_fnirs, _, _ = self.extra_patch_embed2(x_fnirs)
        ...
        # B H*W/256 C
        x_fnirs, _, _ = self.extra_patch_embed3(x_fnirs)
        ...
        # B H*W/1024 C
        x_fnirs, _, _ = self.extra_patch_embed4(x_fnirs)
